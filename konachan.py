...
# 최초 1회 실행 시, X버튼이 제대로 작동 안하는 오류가 있음. 이후에는 잘 꺼짐.
...
#window.iconbitmap(default='icon.ico') # pyinstaller로 실행파일 만들 때, 아이콘 지정하기 때문에 필요는 없음.
#window.tk.call('wm', 'iconphoto', window._w, tk.PhotoImage(file='./icon.png')) # 실행파일로 만들었을 경우, 폴더에 해당 이미지(./icon.png) 를 찾을 수 없으면 에러 발생.
